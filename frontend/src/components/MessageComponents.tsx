...
export function SystemMessageComponent({
  ...
  const getDetails = () => {
    if (
      ...
      // Qwen SDK uses permission_mode instead of permissionMode
      if ("permission_mode" in message) {
        details.push(`Permission Mode: ${message.permission_mode}`);
      }
      return details.join("\n");
